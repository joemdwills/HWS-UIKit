...
//  Project23
//
//  Created by Joe Williams on 06/12/2021.
//
...
class GameScene: SKScene {
    ...
    let xVelocity = [8,15,3,5]
    let yVelocity = [24,32]
    let fastMultipier = 55
    let slowMultiplier = 40
    ...
    func endGame(triggeredByBomb: Bool) {
        ...
        finalScoreLabel = SKLabelNode(fontNamed: "Helvetica Neue")
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        // this section is important because it removes all animations of the shape nodes including any 'fade out animations' from touchesEnded, before drawing new tocuhes
    ...
    func createEnemy(forceBomb: ForceBomb = .random) {
        ...
        let randomPosition = CGPoint(x: Int.random(in: xMinPos...xMaxPos), y: startYPosition)
        ...
        if randomPosition.x < 256 {
            randomXVelocity = Int.random(in: xVelocity[0]...xVelocity[1])
        } else if randomPosition.x < 512 {
            randomXVelocity = Int.random(in: xVelocity[2]...xVelocity[3])
        ...
        let randomYVelocity = Int.random(in: yVelocity[0]...yVelocity[1])
        
        enemy.physicsBody = SKPhysicsBody(circleOfRadius: 64)
        if enemy.name == "fastEnemy" {
            enemy.physicsBody?.velocity = CGVector(dx: randomXVelocity * fastMultipier, dy: randomYVelocity * fastMultipier)
        } else {
            enemy.physicsBody?.velocity = CGVector(dx: randomXVelocity * slowMultiplier, dy: randomYVelocity * slowMultiplier)
        }
